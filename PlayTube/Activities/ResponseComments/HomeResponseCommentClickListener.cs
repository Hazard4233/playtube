...
{
    public class HomeResponseCommentClickListener : Java.Lang.Object, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, MaterialDialog.IInputCallback
    {
        private readonly Activity Context;
        private readonly HomeResponsePlayerFragment MainContext;
        ...
        public void MoreCommentPostClick(CommentObject item)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    var dialogList = new MaterialDialog.Builder(MainContext.Context).Theme(AppSettings.SetTabDarkTheme ? Theme.Dark : Theme.Light);
            ...
            try
            {
                ...
                if (text == Context.GetString(Resource.String.Lbl_CopeText))
                {
                    if (TypeClass == "Comment")
                        Methods.CopyToClipboard((Activity)MainContext.Context, Methods.FunString.DecodeString(CommentObject.Text));
                    else if (TypeClass == "Reply")
                        Methods.CopyToClipboard((Activity)MainContext.Context, Methods.FunString.DecodeString(ReplyObject.Text));
                }
