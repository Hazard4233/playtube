...
namespace PlayTube.Activities.Models
{
    public class CustomNavigationController : Java.Lang.Object, View.IOnClickListener
    {
        ...
        private LinearLayout HomeButton, ProfileButton, TrendButton, BlogButton, ResponseHomeButton;
        private ImageView HomeImage, NotificationImage, TrendImage, BlogImage, ResponseHomeImage;
        public ImageView ProfileImage;
        public TextView txtUnSeenCount;
        ...
        public readonly List<Fragment> FragmentListTab5 = new List<Fragment>();

        private readonly TabbedMainActivity Context;
        private LinearLayout MainLayout;
        private HomeResponsePlayerFragment homeResponse;
        ...
        public void OnClick(View v)
        {
            switch (v.Id)
            {
                case Resource.Id.llResponseHome:
                    EnableNavigationButton(ResponseHomeImage);
                    PageNumber = 5;
                    ShowFragment5();
                    if (Context.MyResponsesFragment.activePage == 1)
                    {
                        if (Context.MyResponsesFragment.ForYouResponses.Player != null)
                            Context.MyResponsesFragment.ForYouResponses.Player.PlayWhenReady = true;
                    } else
                    {
                        if (Context.MyResponsesFragment.FollowingResponses.Player != null)
                            Context.MyResponsesFragment.FollowingResponses.Player.PlayWhenReady = true;
                    }
                    //homeResponse = HomeResponsePlayerFragment.GetInstance();
                    //if (homeResponse != null && !homeResponse.Player.PlayWhenReady)
                    //    homeResponse.Player.PlayWhenReady = true;
                    //Context.MyResponsesFragment.InitTab();
                    break;
                ...
                case Resource.Id.llHome:
                    ...
                    ShowFragment0();
                    if (Context.MyResponsesFragment.FollowingResponses.Player != null)
                        Context.MyResponsesFragment.FollowingResponses.Player.PlayWhenReady = false;
                    if (Context.MyResponsesFragment.ForYouResponses.Player != null)
                        Context.MyResponsesFragment.ForYouResponses.Player.PlayWhenReady = false;
                    //homeResponse = HomeResponsePlayerFragment.GetInstance();
                    //if (homeResponse != null && homeResponse.Player.PlayWhenReady)
                    //    homeResponse.Player.PlayWhenReady = false;
                    break;
                ...
                case Resource.Id.llProfile:
                    ...
                    //Context.MyChannelFragment.InitTab();
                    break;
            }
        }
        ...
        public void DisableAllNavigationButton()
        {
            ...
            ResponseHomeImage.Background = null;
            ResponseHomeImage.SetColorFilter(AppSettings.SetTabDarkTheme ? Color.White : Color.Black);
        ...
        public Fragment GetSelectedTabBackStackFragment()
        {
            switch (PageNumber)
            {
                ...
                case 5:
                    {
                        var currentFragment = FragmentListTab5[FragmentListTab5.Count - 2];
                        if (currentFragment != null)
                            return currentFragment;
                        break;
                    }
        ...
        public int GetCountFragment()
        {
            try
            {
                switch (PageNumber)
                {
                    ...
                    case 5:
                        return FragmentListTab5.Count > 1 ? FragmentListTab5.Count : 0;
                    default:
                        return 0;
                }
            }
        ...
        public void DisplayFragment(Fragment newFragment)
        {
            try
            {
                ...
                HideFragmentFromList(FragmentListTab5, ft);
                ...
                if (PageNumber == 5)
                    if (!FragmentListTab5.Contains(newFragment))
                        FragmentListTab5.Add(newFragment);
        ...
        private void RemoveFragment(Fragment oldFragment)
        {
            try
            {
                ...
                if (PageNumber == 5)
                    if (FragmentListTab5.Contains(oldFragment))
                        FragmentListTab5.Remove(oldFragment);
        ...
        public void ShowFragment5()
        {
            try
            {
                if (FragmentListTab5.Count <= 0) return;
                var currentFragment = FragmentListTab5[FragmentListTab5.Count - 1];
                if (currentFragment != null)
                    DisplayFragment(currentFragment);
