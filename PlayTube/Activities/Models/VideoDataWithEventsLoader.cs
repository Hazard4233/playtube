...
{
    public class VideoDataWithEventsLoader : Java.Lang.Object, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, MaterialDialog.IInputCallback, TabLayout.IOnTabSelectedListener
    {
        public readonly TabbedMainActivity ActivityContext;
        private TextView ShareIconView, AddToIconView, ShowMoreDescriptionIconView, TextChannelName, EditIconView, TxtSubscribeCount;
        public ImageView ImageChannelView, LikeIconView, UnLikeIconView, ViewMoreCommentSection;
        ...
        public VideoObject VideoDataHandler;
        private readonly Activity Context;
        public VideoEnumTypes VideoType;
        private LinearLayout PaymentLayout;
        private LinearLayout DonateButton, RentButton;
        private string TypeDialog;
        public Switch AutoNextswitch;
        private FrameLayout VideoButtomLayout;
        public RelativeLayout CommentButtomLayout, CircleLayout;
        private Button JoinButton;
        public TextView txtHideComment;
        ...
        public void SetViews()
        {
            try
            {
                ...
                JoinButton = ActivityContext.FindViewById<Button>(Resource.Id.JoinButton);
                JoinButton.Click += JoinButton_Click;
                txtHideComment = ActivityContext.FindViewById<TextView>(Resource.Id.txtHideComment);
                ...
                btnAddAudioResponse = ActivityContext.FindViewById<Button>(Resource.Id.btnAddAudioResponse);
                btnAddVideoResponse = ActivityContext.FindViewById<Button>(Resource.Id.btnAddVideoResponse);
                ...
                tabLayout = (TabLayout)ActivityContext.FindViewById(Resource.Id.tabs);
                tabLayout.AddTab(tabLayout.NewTab().SetText("Response"), 0, true);
                tabLayout.AddTab(tabLayout.NewTab().SetText("Related Video"), 1, false);
                viewPager = (ViewPager)ActivityContext.FindViewById(Resource.Id.viewpager);
                LinearLayoutManager mLayoutManager = new LinearLayoutManager(ActivityContext);
                mLayoutManager.Orientation = LinearLayoutManager.Vertical;
                //mRecyclerView.setLayoutManager(mLayoutManager);
                NextFragment = new NextToFragment();
                ResponseFragment = new UserResponseFragment();
                viewPager.Adapter = new UserReponse.PagerAdapter(ActivityContext.SupportFragmentManager, tabLayout.TabCount, NextFragment, ResponseFragment);
        ...
        private void TxtHideComment_Click(object sender, EventArgs e)
        {
            ...
            ftvideo.AddToBackStack(null);
            ftvideo.SetCustomAnimations(Resource.Animation.slide_up, Resource.Animation.slide_down);
            ftvideo.Hide(ActivityContext.CommentsFragment).Commit();
        }
        ...
        private void JoinButton_Click(object sender, EventArgs e)
        {
            ...
            {
                UserResponsePopup mFragment = new UserResponsePopup(VideoDataHandler.VideoId, this.ActivityContext.VideoActionsController, ResponseFragment);
                mFragment.Show(ActivityContext.SupportFragmentManager, mFragment.Tag);
            }
            else
            {
                Snackbar.Make(this.ActivityContext.Window.DecorView, "You are not logged in!", Snackbar.LengthLong);
            }
        }
        ...
        private void BtnAddVideoResponse_Click(object sender, EventArgs e)
        {
            ...
            UserResponseBottomSheet mFragment = new UserResponseBottomSheet(UserResponse.Video, VideoDataHandler.VideoId, this.ActivityContext.VideoActionsController, ResponseFragment);
            mFragment.Show(ActivityContext.SupportFragmentManager, mFragment.Tag);
        }

        private void BtnAddAudioResponse_Click(object sender, EventArgs e)
        {
            UserResponseBottomSheet mFragment = new UserResponseBottomSheet(UserResponse.Audio, VideoDataHandler.VideoId, this.ActivityContext.VideoActionsController, ResponseFragment);
            mFragment.Show(ActivityContext.SupportFragmentManager, mFragment.Tag);
        }
        ...
        private void CommentButtomLayout_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ftvideo.SetCustomAnimations(Resource.Animation.slide_up, Resource.Animation.slide_down);
                    ftvideo.Show(ActivityContext.CommentsFragment).Commit();
                }
        ...
        private void RentButtonOnClick(object sender, EventArgs e)
        {
            try
            {
                if (AppSettings.ShowInAppBilling && PlayTubeClient.Client.IsExtended && AppSettings.VideoRentalPriceStatic)
                {
        ...
        public async void LoadVideoData(VideoObject videoObject)
        {
            try
            {
                UpNextTextview.Text = ActivityContext.GetString(Resource.String.Lbl_NextTo);
                ...
                ///var response = await UserResponseAPI.GetUserResponse(videoObject.VideoId);
                ...
                VideoPublishDate.Text = ActivityContext.GetText(Resource.String.Lbl_Published_on) + " " + videoObject.TimeDate;
                ...
                if (ActivityContext.CommentsFragment != null && ActivityContext.CommentsFragment.MAdapter != null)
                {
                    ...
                    //ActivityContext.UserResponseTabs.NextFragment.MAdapter.VideoList?.Clear();
                    ActivityContext.CommentsFragment.MAdapter.NotifyDataSetChanged();
                    //ActivityContext.UserResponseTabs.NextFragment.MAdapter.NotifyDataSetChanged();
            ...
            try
            {
                ...
                if (apiStatus == 200)
                {
                    if (respond is GetVideosDetailsObject result)
                    {
                        ...
                        NextFragment.LoadDataAsync(new ObservableCollection<VideoObject>(result.DataResult.SuggestedVideos));
